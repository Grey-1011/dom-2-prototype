...
window.$ = window.jQuery = function (selectorOrArrayOrTemplate) {
    ...
    var api = Object.create(jQuery.prototype); // 创建一个对象，这个对象的__proto__为括号里的东西
...
jQuery.fn = jQuery.prototype = {
    ...
    appendTo: function appendTo(node) {
        if (node instanceof Element) {
            ...
            }); //遍历elements,对每个el,进行node.appendChild 操作
        } else if (node.jQuery === true) {
            this.each(function (el) {
                return node.get(0).appendChild(el);
            }); // 遍历elements,对每个el,进行node.get(0).appendChild(el) 操作
        }
    },
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + '60315' + '/');
...
//# sourceMappingURL=/jquery.f6a56818.map